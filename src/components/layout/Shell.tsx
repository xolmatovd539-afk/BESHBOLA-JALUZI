...
export function Shell({ children }: ShellProps) {
  ...
  return (
    <div className="h-screen flex flex-col font-sans text-slate-800 liquid-bg overflow-hidden p-0 lg:p-4 gap-4">
      ...
      <div className="flex-1 flex overflow-hidden gap-4">
        ...
        <main className="flex-1 flex flex-col min-w-0 bg-transparent relative overflow-hidden">
          {/* Header */}
          <header className="h-20 lg:h-24 glass rounded-[2.5rem] flex items-center justify-between px-6 lg:px-10 flex-shrink-0 mb-4 animate-in fade-in slide-in-from-top duration-700">
            <div className="flex items-center gap-4">
              <button 
                onClick={() => setIsMobileMenuOpen(true)}
                className="lg:hidden p-3 -ml-2 text-slate-600 hover:bg-white/50 rounded-2xl"
              >
                <Menu size={28} />
              </button>
              <div>
                <h1 className="text-xl lg:text-2xl font-black tracking-tight text-slate-900 leading-none">
                  <span className="text-red-600">BESHBOLA JALUZI</span> <span className="text-indigo-600">Visualizer</span>
                </h1>
                <p className="text-[10px] lg:text-[11px] text-slate-400 uppercase tracking-widest font-black mt-2">Studiya Maishiy • Muallif: {authorName}</p>
              </div>
            </div>
            
            <div className="flex space-x-3 lg:space-x-4 items-center">
              <div className="hidden sm:flex items-center space-x-3 bg-white/40 border border-white/50 px-5 py-2.5 rounded-2xl shadow-sm text-indigo-600 text-[11px] lg:text-xs font-black tracking-tight">
                <span className="w-2 h-2 bg-indigo-600 rounded-full animate-pulse"></span>
                <span>$1 = {globalRate.toLocaleString()} UZS</span>
              </div>
              <button className="bg-slate-900 text-white px-6 py-3 rounded-2xl hover:bg-slate-800 text-xs lg:text-sm font-black transition-all shadow-xl active:scale-95">
                Saqlash
              </button>
            </div>
          </header>
          ...
          {/* Page Content */}
          <div className="flex-1 glass rounded-[2.5rem] overflow-hidden flex flex-col relative animate-in fade-in zoom-in-95 duration-1000">
            <div className="flex-1 overflow-y-auto">
              {children}
            </div>
            
            {/* Bottom Alert Bar Integrated into Content Box */}
            <footer className="h-12 lg:h-14 bg-indigo-600/90 backdrop-blur-md text-white flex items-center px-8 space-x-8 overflow-hidden whitespace-nowrap flex-shrink-0">
              <div className="flex items-center space-x-3 flex-shrink-0">
                ...
                <span className="text-xs font-bold tracking-tight">Kurs yangilanish {lastUpdated || 'hozirgina'}</span>
              </div>
              <div className="w-px h-5 bg-white/20 hidden lg:block"></div>
              <div className="flex items-center space-x-2">
                <span className="text-[10px] font-black uppercase tracking-widest leading-none hidden sm:block">Foydalanuvchi:</span>
                <span className="text-xs font-bold tracking-tight italic opacity-80">{authorName}</span>
              </div>
            </footer>
          </div>
        </main>
